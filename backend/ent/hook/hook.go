...
// Mutate calls f(ctx, m).
func (f MealFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.MealMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MealMutation", m)
	}
	return f(ctx, mv)
}

// The MealplanFunc type is an adapter to allow the use of ordinary
// function as Mealplan mutator.
type MealplanFunc func(context.Context, *ent.MealplanMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f MealplanFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.MealplanMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MealplanMutation", m)
	}
	return f(ctx, mv)
}
