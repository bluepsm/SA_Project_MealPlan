...
func (uq *UserQuery) QueryMealplans() *MealplanQuery {
	query := &MealplanQuery{config: uq.config}
...
func (uq *UserQuery) QueryFoods() *FoodQuery {
	query := &FoodQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithMealplans(opts ...func(*MealplanQuery)) *UserQuery {
	query := &MealplanQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withMealplans = query
	return uq
}
...
func (uq *UserQuery) WithFoods(opts ...func(*FoodQuery)) *UserQuery {
	query := &FoodQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withMealplans; query != nil {
		...
		query.Where(predicate.Mealplan(func(s *sql.Selector) {
			s.Where(sql.InValues(user.MealplansColumn, fks...))
		}))
	...
	if query := uq.withFoods; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.withFKs = true
		query.Where(predicate.Food(func(s *sql.Selector) {
			s.Where(sql.InValues(user.FoodsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.Foods = append(node.Edges.Foods, n)
		}
	}
